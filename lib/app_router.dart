...
GoRouter createRouter(AuthRepository auth, BudgetController budget) => GoRouter(
  ...
  routes: [
    ...
    GoRoute(path: '/history', builder: (c, s) => const HistoryScreen()), // ✅ agregado
  ...
  redirect: (context, state) {
    ...
    final isAuthLoc = loc == '/login' || loc == '/register';

    if (loc == '/splash') return loggedIn ? '/home' : '/login';

    if (!loggedIn) return isAuthLoc ? null : '/login';

    // onboarding mínimo: necesitamos al menos ingresos
